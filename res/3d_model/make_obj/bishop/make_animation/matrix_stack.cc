...
namespace matrix_stack
{
	...
	Vector4 operator+(const Vector4 &p0, const Vector4 &p1)
	{
		return Vector4(p0[0] + p1[0], p0[1] + p1[1], p0[2] + p1[2]);
	}

	Vector4 operator*(const Vector4 &p, double k)
	{
		return Vector4(p[0] * k, p[1] * k, p[2] * k);
	}
